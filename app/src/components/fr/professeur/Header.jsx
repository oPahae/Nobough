...
const Header = ({ session, activeTab, setActiveTab, headerOpened, setHeaderOpened, formationID, setFormationID }) => {
    ...
    useEffect(() => {
        if (activeTab === "formation" && !currentFormationID) {
            setCurrentFormationID(formationID)
            localStorage.setItem('enseignementID', formationID)
            setTimeout(() => {
    ...
    const closeFormation = (e) => {
        ...
        gsap.to(recent.current, {
            ...
            ease: "power3.in",
            onComplete: () => {
                localStorage.removeItem('enseignementID')
    ...
    useEffect(() => {
        const recentFormationID = localStorage.getItem('enseignementID')
    ...
    useEffect(() => {
        const active = localStorage.getItem('activeTabProfesseur')
        setActiveTab(active)
    }, [])
    ...
    const fetchNotifs = async () => {
        try {
            const res = await fetch(`/api/notifications/getProf?profID=${session.id}`);
    ...
    const setNotificationsRead = async () => {
        try {
            const res = await fetch(`/api/notifications/setReadProf?profID=${session.id}`);
            const data = await res.json();
    ...
    const handleLogout = async () => {
        const res = await fetch('/api/_auth/profLogout', { method: 'POST' });
        if (res.ok) {
                                        ...
                                        {notifications.map((notification, index) => (
                                            <div key={index} className="px-4 py-3 hover:bg-amber-50 border-b border-amber-50 last:border-b-0 transition-colors duration-200">
                                                <div className="flex items-start">
                                                    <div className={`p-1 rounded-full mr-3 mt-1 shadow-sm ${notification.type === 'adhesion' ? 'bg-green-100' :
                                                        notification.type === 'abandon' ? 'bg-red-100' : 'bg-gray-100'
                                                        }`}>
                                                        {notification.type === 'adhesion' && <LogIn className="w-4 h-4 text-green-500" />}
                                                        {notification.type === 'abandon' && <LogOut className="w-4 h-4 text-red-500" />}
                                                    </div>
                                                    <div>
                ...
                <div className="bg-white shadow-xl md:hidden z-50 rounded-b-lg overflow-hidden border-t border-amber-100">
                    <ul className="flex flex-col">
                        {currentFormationID && (
                            <li>
                                <button
                                    className={`px-4 py-3 font-medium transition-all w-full text-left flex items-center justify-between ${activeTab === 'formation' ? 'bg-amber-50 text-amber-800' : 'hover:bg-amber-50'}`}
                                    onClick={() => handleTabChange('formation')}
                                >
                                    <div className="flex items-center">
                                        <PieChart className={`w-5 h-5 mr-3 ${activeTab === 'formation' ? 'text-amber-600' : 'text-gray-500'}`} />
                                        <span>Récent</span>
                                    </div>
                    ...
                    <ul className="flex overflow-x-auto">
                        {currentFormationID &&
                            <li ref={recent} className="translate-y-20">
                                <button
                                    className={`px-5 py-4 font-medium border-b-2 transition-all duration-300 flex items-center ${activeTab === 'formation' ? 'border-amber-600 text-amber-800 bg-amber-50' : 'border-transparent hover:bg-amber-50'} cursor-pointer`}
                                    onClick={() => setActiveTab('formation')}
                                >
                                    <div className="bg-amber-100 rounded-full p-2 mr-2">
                                        <PieChart className={`w-4 h-4 ${activeTab === 'formation' ? 'text-amber-600' : 'text-gray-500'}`} />
                                    </div>
