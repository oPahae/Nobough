    ...
    return (
        <div className={`fixed top-0 left-0 w-full z-40 ${!headerOpened && 'hidden'}`}>
            <header className="relative text-gray-800 p-4 shadow-md" style={{
                ...
                <div className="absolute top-0 left-0 right-0 h-6 overflow-hidden opacity-20 z-10">
                        ...
                        {session &&
                            <>
                                <div className="relative">
                                    ...
                                    >
                                        ...
                                        {notifications.filter(notif => notif.vue == 0).length > 0 &&
                                            <span className="absolute -top-1 -right-1 bg-white text-amber-700 rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold shadow-md">
                                    ...
                                    {showNotifications && (
                                        <div className="absolute -right-26 md:right-0 mt-3 w-72 bg-white rounded-lg shadow-xl py-2 z-50 border border-amber-100" onClick={e => e.stopPropagation()}>
                                            <div className="px-4 py-2 border-b border-amber-100 flex items-center">
                                                <h3 className="font-medium text-gray-800">Notifications</h3>
                                                <div className="ml-2 h-6 w-6 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold">
                                                ...
                                                {notifications.map((notification, index) => (
                                                    <div key={index} className="px-4 py-3 hover:bg-amber-50 border-b border-amber-50 last:border-b-0 transition-colors duration-200">
                                                        <div className="flex items-start">
                                                            <div className={`p-1 rounded-full mr-3 mt-1 shadow-sm ${notification.type === 'validation' ? 'bg-green-100' :
                                ...
                                <div className="relative">
                                    ...
                                    {showProfileMenu && (
                                        <div className="absolute right-0 mt-3 w-48 bg-white rounded-lg shadow-xl py-2 z-50 border border-amber-100" onClick={e => e.stopPropagation()}>
                                            <button onClick={() => setActiveTab('profile')} className="w-full text-left px-4 py-3 text-gray-700 hover:bg-amber-50 flex items-center cursor-pointer transition-colors duration-200">
                                                <User className="w-4 h-4 mr-3 text-amber-600" />
                                                <span>Profil</span>
                                            </button>
                                            <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-gray-700 hover:bg-amber-50 flex items-center cursor-pointer transition-colors duration-200">
                    ...
                    <div className="bg-white shadow-xl md:hidden z-50 rounded-b-lg overflow-hidden border-t border-amber-100">
                        <ul className="flex flex-col">
                            {!session && (
                                <li>
                                    <button
                                        className={`px-4 py-3 font-medium transition-all w-full text-left flex items-center justify-between ${activeTab === 'register' ? 'bg-amber-50 text-amber-800' : 'hover:bg-amber-50'}`}
                                        onClick={() => handleTabChange('register')}
                                    >
                                        <div className="flex items-center">
                                            <LogIn className={`w-5 h-5 mr-3 ${activeTab === 'register' ? 'text-amber-600' : 'text-gray-500'}`} />
                            ...
                            {currentFormationID && (
                                <li>
                                    <button
                                        className={`px-4 py-3 font-medium transition-all w-full text-left flex items-center justify-between ${activeTab === 'etudier' ? 'bg-amber-50 text-amber-800' : 'hover:bg-amber-50'}`}
                                        onClick={() => handleTabChange('etudier')}
                                    >
                                        <div className="flex items-center">
                                            <PieChart className={`w-5 h-5 mr-3 ${activeTab === 'etudier' ? 'text-amber-600' : 'text-gray-500'}`} />
                                            <span>Récent</span>
                                        </div>
                            ...
                            {navItems.map((item) => (
                                <li key={item.id}>
                                    <button
                                        className={`px-4 py-3 font-medium transition-all w-full text-left flex items-center ${activeTab === item.id ? 'bg-amber-50 text-amber-800' : 'hover:bg-amber-50'} ${['Room', 'Payer', 'Documents'].includes(item.label) && !session && 'bg-gray-200 line-through'}`}
                                        onClick={() => handleTabChange(item.id)}
                                        disabled={['Room', 'Payer', 'Documents'].includes(item.label) && !session}
                                    >
                                        <item.icon className={`w-5 h-5 mr-3 ${activeTab === item.id ? 'text-amber-600' : 'text-gray-500'}`} />
                                        <span>{item.label}</span>
                                        {['Room', 'Payer', 'Documents'].includes(item.label) && !session && <Lock className='text-xs rounded-full font-bold text-white bg-gradient-to-br from-gray-400 to-gray-500 p-1 ml-2' />}
                                    </button>
                                </li>
                            ))}
                ...
                <div className="container mx-auto">
                    <ul className="flex overflow-x-auto">
                        {!session &&
                            <li>
                                ...
                                >
                                    ...
                                    <span className="hidden ml-1 lg:block">S'authentifier</span>
                                </button>
                            </li>
                        }
                        ...
                        {currentFormationID &&
                            <li ref={recent} className="translate-y-20">
                                ...
                                >
                                    ...
                                    <span className="hidden ml-1 lg:block">Récent</span>
                                    <div className="ml-2 rounded-full hover:bg-red-100 p-1 transition-colors duration-200">
                        ...
                        {navItems.map((item) => (
                            <li key={item.id}>
                                <button
                                    className={`px-5 py-4 font-medium border-b-2 transition-all duration-300 flex items-center ${activeTab === item.id ? 'border-amber-600 text-amber-800 bg-amber-50' : 'border-transparent hover:bg-amber-50'} cursor-pointer ${['Room', 'Payer', 'Documents'].includes(item.label) && !session && 'bg-gray-200 line-through'}`}
                                ...
                                >
                                    ...
                                    {['Room', 'Payer', 'Documents'].includes(item.label) && !session && <Lock className='text-sm rounded-full font-bold text-white bg-gradient-to-br from-gray-400 to-gray-500 p-1 -translate-y-4' />}
                                </button>
                            </li>
                        ))}
                    </ul>
