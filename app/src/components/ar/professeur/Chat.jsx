...
const Chat = ({ formationID, session }) => {
    ...
    const fetchMessages = async () => {
        try {
            ...
            if (!response.ok) {
                throw new Error('خطأ أثناء استرجاع الرسائل')
            }
            ...
            console.error('خطأ أثناء استرجاع الرسائل:', error)
    ...
    return (
        <div className="max-w-full relative md:px-22" dir="rtl">
            ...
            <div className="mb-12 text-center pt-8 relative">
                <div className="inline-block">
                    ...
                    <h1 className="text-3xl font-bold text-gray-800">الدردشة المباشرة</h1>
                ...
                <p className="text-gray-600 mt-4 max-w-xl mx-auto">تبادل الرسائل مع طلابك</p>
            </div>

            <div className="bg-white rounded-xl shadow-xl mb-6 overflow-hidden relative">
                <PatternDecoration />
                <div className="flex flex-col h-screen max-h-[70vh]">
                    <div className="flex-grow overflow-y-auto p-6 bg-white/90">
                        {messages.map((msg) => (
                            <div key={msg.id} className={`mb-6 ${msg.role === 'professeur' ? 'flex justify-start' : 'flex justify-end'}`}>
                                {msg.role !== 'professeur' && (
                                    <div className="ml-3">
                                ...
                                <div className={`max-w-[75%] ${msg.role === 'professeur' ? 'order-2' : 'order-1'}`}>
                                    {true && (
                                        <div className="flex items-center mb-1.5 mr-1">
                                            <span className="font-semibold text-sm ml-2 text-gray-700">
                                                {msg.emetteur}
                                            </span>
                                            {msg.role === 'professeur' && (
                                    ...
                                    <div className={`rounded-2xl p-4 shadow-md ${msg.role === 'professeur'
                                        ? 'bg-gradient-to-l from-amber-600 to-amber-800 text-white rounded-bl-none'
                                        : 'bg-amber-50 border border-amber-200 text-gray-800 rounded-br-none'
                                        }`}>
                                        <p className="break-words leading-relaxed">{msg.contenu}</p>
                                        <div className={`text-xs mt-2 ${msg.role === 'professeur' ? 'text-amber-200' : 'text-gray-500'} text-left`}>
                    ...
                    <form onSubmit={handleSendMessage} className="border-t border-amber-100 p-6 bg-white">
                        <div className="flex gap-3 items-center">
                            ...
                            <button
                                type="submit"
                                className="p-4 bg-gradient-to-l from-amber-600 to-amber-800 text-white rounded-xl shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 cursor-pointer flex items-center justify-center"
                            >
