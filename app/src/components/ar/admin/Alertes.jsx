    ...
    const analyserDettesAvecIA = async (budget, dettes) => {
        ...
        try {
            ...
            const inputData = features.map(f => [
                normaliser(Math.max(30 - f.joursRestants, 0), 0, 30),
                normaliser(f.anciennete, 0, maxAnciennete),
                f.montant <= budget.reel ? 1.0 : 0.0,
                normaliser(f.ratioBudget, 0, 3)
            ]);
        ...
        const chargerModelEtAnalyser = async () => {
            ...
            if (budget.reel < 1000) {
                nouvellesAlertes.push({
                    type: 'danger',
                    message: 'الميزانية الحقيقية معرضة لأن تصبح سلبية! انتبه للنفقات الإضافية.',
            ...
            try {
                ...
                if (recommandationsIA.length > 0) {
                    ...
                    if (dettePrioritaire.montant <= budget.reel) {
                        nouvellesAlertes.push({
                            type: 'info',
                            message: `توصية نبوغ: سدد أولويةً "${dettePrioritaire.label}" (${dettePrioritaire.montant} درهم)`,
                    ...
                    recommandationsIA.forEach(dette => {
                        if (dette.joursRestants <= 7 && dette.joursRestants > 0) {
                            nouvellesAlertes.push({
                                type: 'warning',
                                message: `الموعد النهائي قريب: يجب دفع "${dette.label}" خلال ${dette.joursRestants} أيام`,
                                icon: <Calendar className="w-5 h-5 mt-0.5 flex-shrink-0" />
                            });
                        } else if (dette.joursRestants === 0) {
                            nouvellesAlertes.push({
                                type: 'danger',
                                message: `الموعد النهائي اليوم لـ "${dette.label}" (${dette.montant} درهم)`,
                                icon: <Calendar className="w-5 h-5 mt-0.5 flex-shrink-0" />
                            });
                        }
                    });
                ...
                console.error("خطأ أثناء التحليل بواسطة الذكاء الاصطناعي:", error);
                nouvellesAlertes.push({
                    type: 'danger',
                    message: "خطأ في تحميل نموذج الذكاء الاصطناعي. يتم استخدام الوضع الاحتياطي.",
                    icon: <AlertCircle className="w-5 h-5 mt-0.5 flex-shrink-0" />
                });
            }
        ...
        <div className="space-y-4" dir="rtl">
            {alertes.length > 0 && (
                <div className="container mx-auto mt-4">
                    <h2 className="text-lg font-semibold mb-2">تنبيهات</h2>
            ...
            {recommandations.length > 0 ? (
                <div className="container mx-auto mt-2">
                    <div className="flex items-center mb-2">
                        <h2 className="text-lg font-semibold">توصيات السداد (الذكاء الاصطناعي)</h2>
                        {modelInitialized && (
                            <span className="mr-2 px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                                نموذج تنسورفلو نشط
                            </span>
                        )}
                    </div>
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الدين</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">المبلغ</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الموعد النهائي</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">درجة الذكاء الاصطناعي</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                                {recommandations.map((dette) => (
                                    <tr key={dette.id} className={dette.montant <= budget.reel ? "bg-green-50" : ""}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{dette.montant} درهم</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {dette.joursRestants === 0 ?
                                                <span className="text-red-600 font-medium">اليوم</span> :
                                                `${dette.joursRestants} أيام`}
                                        </td>
                ...
                <div className="container mx-auto mt-2 p-4 bg-gray-50 rounded-lg text-center">
                    {modelInitialized ?
                        <p>لا يوجد ديون للسداد في الوقت الحالي.</p> :
                        <p>جاري تحميل نموذج الذكاء الاصطناعي لتحليل الديون...</p>
                    }
                </div>
            )}
        </div>
