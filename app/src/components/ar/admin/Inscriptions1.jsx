        ...
        try {
            ...
            if (response.ok) {
                setNotification((notif) => ({
                    ...notif,
                    msg: `تم التحقق من التسجيل رقم #${id} بنجاح!`,
                    type: 'success',
                    shown: true
                }))
                fetchInscriptions()
                ...
                setNotification((notif) => ({
                    ...notif,
                    msg: data.message || 'خطأ أثناء التحقق من التسجيل.',
                    type: 'error',
                    shown: true
                }))
            }
    ...
    return (
        <div className="max-w-full relative" dir="rtl">
            ...
            <div className="mb-12 text-center pt-8 relative">
                ...
                <p className="text-gray-600 mt-4 max-w-xl mx-auto">قم بتأكيد أو رفض طلبات التسجيل للأعضاء الجدد</p>
            </div>
            ...
            <div className="max-w-5xl mx-auto bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-amber-100">
                <div className="p-6 border-b border-amber-100 bg-gradient-to-l from-amber-50 to-amber-100/40">
                    <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-bl from-amber-400 to-amber-700 flex items-center justify-center shadow-md">
                            <UserCheck className="w-5 h-5 text-white" />
                        </div>
                        <h2 className="text-xl font-semibold mr-3 text-amber-900">تسجيلات جديدة ({inscriptions ? inscriptions.length : 0})</h2>
                    </div>
                </div>
                ...
                <div className="divide-y divide-amber-100">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                                <div className="flex mb-4 sm:mb-0">
                                    <div className="relative">
                                        ...
                                        />
                                        <div className="absolute -bottom-1 -left-1 w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center border border-amber-300">
                                    ...
                                    <div className="mr-4">
                                        ...
                                        <div className="flex items-center text-sm text-gray-600">
                                            <Calendar className="w-3 h-3 ml-1 text-amber-700" />
                                            مسجل في {new Date(inscription.created_At).toLocaleDateString('FR-fr')}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center space-x-3 w-full sm:w-auto">
                                    <button
                                        className="py-2 px-4 flex-1 sm:flex-none bg-gradient-to-l from-green-500 to-green-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 flex items-center justify-center cursor-pointer"
                                    ...
                                    {showRefuseReason[inscription.id] ? (
                                        <div className="flex items-center flex-1 sm:flex-none">
                                            <input
                                                type="text"
                                                className="py-2 px-4 border border-amber-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-500 ml-2 w-full"
                                                placeholder="سبب الرفض..."
                                                onChange={(e) => setRefuseReason({ ...refuseReason, [inscription.id]: e.target.value })}
                                                value={refuseReason[inscription.id] || ""}
                                            />
                                            <button
                                                className="py-2 px-4 bg-gradient-to-l from-red-500 to-red-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 flex items-center justify-center cursor-pointer disabled:opacity-50"
                                        ...
                                        <button
                                            className="py-2 px-4 flex-1 sm:flex-none bg-gradient-to-l from-gray-200 to-gray-300 text-gray-700 rounded-lg shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 flex items-center justify-center cursor-pointer"
                            ...
                            <details className="mt-6">
                                ...
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-amber-50/40 rounded-xl border border-amber-100">
                                    <div className="bg-white p-4 rounded-lg shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex flex-col sm:flex-row sm:items-center">
                                                <span className="font-medium text-sm text-gray-700 sm:w-1/3">الاسم الكامل:</span>
                                                <span className="text-gray-600 sm:w-2/3">{inscription.prenom} {inscription.nom}</span>
                                            </div>
                                            <div className="flex flex-col sm:flex-row sm:items-center">
                                                <span className="font-medium text-sm text-gray-700 sm:w-1/3">تاريخ الميلاد:</span>
                                                <span className="text-gray-600 sm:w-2/3">{new Date(inscription.birth).toLocaleDateString('FR-fr')}</span>
                                            </div>
                                            <div className="flex flex-col sm:flex-row sm:items-center">
                                                <span className="font-medium text-sm text-gray-700 sm:w-1/3">رقم الهوية:</span>
                                    ...
                                    <div className="bg-white p-4 rounded-lg shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex flex-col sm:flex-row sm:items-center">
                                                <span className="font-medium text-sm text-gray-700 sm:w-1/3">البريد الإلكتروني:</span>
                                                <span className="text-gray-600 sm:w-2/3">{inscription.email}</span>
                                            </div>
                                            <div className="flex flex-col sm:flex-row sm:items-center">
                                                <span className="font-medium text-sm text-gray-700 sm:w-1/3">الهاتف:</span>
                    ...
                    {(!inscriptions || inscriptions.length === 0) && (
                        <div className="p-16 text-center">
                            ...
                            <h3 className="text-xl font-bold mb-2 text-amber-900">لا يوجد تسجيلات معلقة</h3>
                            <p className="text-gray-600">لا يوجد طلبات تسجيل جديدة لمعالجتها في الوقت الحالي.</p>
                        </div>
                    )}
                </div>
            </div>
