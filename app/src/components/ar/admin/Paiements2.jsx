...
const Paiements2 = ({ setSelectedImage, paiements, setNotification, fetchPaiements }) => {
    ...
    const handleValiderPaiement = async (id, email, formationID) => {
        try {
            const response = await fetch('/api/inscriptions/validerPaiement', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                setNotification((notif) => ({
                    ...notif,
                    msg: `تم التحقق من الدفع رقم #${id} بنجاح!`,
                    type: 'success',
                    shown: true
                }))
                fetchPaiements()
                envoyerEmail('valider', email, formationID)
            } else {
    ...
    const handleRefuserPaiement = async (id, email, raison) => {
        try {
            const response = await fetch('/api/inscriptions/refuserPaiement', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                setNotification((notif) => ({
                    ...notif,
                    msg: `تم رفض الدفع رقم #${id} : ${raison}`,
                    type: 'info',
                    shown: true
                }))
    ...
    const envoyerEmail = async (type, email, param2) => {
        try {
            const res = await fetch(`/api/_mail/${type}Paiement2`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: type === 'valider' ? JSON.stringify({ email, formationID: param2 }) : JSON.stringify({ email, raison: param2 }),
            })
        ...
        <div dir="rtl">
            ...
            <div className="mb-12 text-center pt-8 relative">
                <div className="inline-block">
                    ...
                    <h1 className="text-3xl font-bold text-gray-800">إدارة الدفع</h1>
                ...
                <p className="text-gray-600 mt-4 max-w-xl mx-auto">إدارة طلبات التسجيل والتحقق من معاملات الدورات التدريبية</p>
            </div>

            <div className="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-xl mb-6 relative">
                <div className="relative flex items-center justify-center mb-6">
                    ...
                    <div className="flex items-center">
                        ...
                        <h2 className="text-2xl font-bold text-amber-900">دفعات الشهر الأول ({paiements.length})</h2>
                    </div>
                        ...
                        <div key={paiement.id} className="bg-white rounded-xl shadow-md overflow-hidden border border-amber-100 hover:shadow-lg group relative">
                            <div className="flex flex-col sm:flex-row justify-between p-6">
                                <div className="flex mb-4 sm:mb-0">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-gray-600 text-sm">دفع بتاريخ {new Date(paiement.created_At).toLocaleDateString('FR-fr')}</p>
                                        <div className="flex items-center mt-2 bg-amber-50 px-3 py-1 rounded-full inline-block">
                                            <div className="flex items-center text-amber-800">
                                                <Book className="w-4 h-4 text-amber-700 ml-2" />
                                                <span className="font-medium">{paiement.formationTitre}</span>
                                            </div>
                                            <span className="mx-2 text-amber-300">•</span>
                                            <span className="font-medium text-amber-700">{paiement.formationPrix} درهم</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center space-x-2 sm:mt-0 mt-4">
                                    <button
                                        ...
                                        onClick={() => handleValiderPaiement(paiement.id, paiement.email, paiement.formationID)}
                                    >
                            ...
                            <details className="px-6 pb-6 border-t border-amber-100">
                                ...
                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            ...
                                            <div className="flex items-center bg-amber-50/50 p-2 rounded-lg">
                                                ...
                                                <span className="mr-2 text-gray-700">{new Date(paiement.birth).toLocaleDateString('FR-fr')}</span>
                                            </div>
                                            <div className="flex items-center bg-amber-50/50 p-2 rounded-lg">
                                    ...
                                    <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                                        <h4 className="font-medium text-amber-800 mb-3 flex items-center">
                                            <Layers className="w-4 h-4 text-amber-700 ml-2" />
                                            تفاصيل الدورة التدريبية
                                        </h4>
                                        <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
                                            <div className="flex items-center mb-3">
                                                <Book className="w-5 h-5 text-amber-700 ml-2" />
                                                <h5 className="font-semibold text-amber-800">{paiement.formationTitre}</h5>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 text-sm">
                                                <div className="col-span-2 flex items-center bg-white/60 p-2 rounded-lg">
                                                    <DollarSign className="w-4 h-4 text-amber-600 ml-2" />
                                                    <span className="font-medium">السعر:</span>
                                                    <span className="mr-2 text-gray-700">{paiement.formationPrix}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            <div className="flex items-center bg-amber-50/50 p-2 rounded-lg">
                                                ...
                                                <span className="mr-2 text-gray-700">{new Date(paiement.created_At).toLocaleDateString('FR-fr')}</span>
                                            </div>
                                            <div className="bg-amber-50/50 p-2 rounded-lg">
                                                <p className="font-medium flex items-center mb-2">
                                                    <FileText className="w-4 h-4 text-amber-600 ml-2" />
                                                    الرسالة:
                                                </p>
                                                <p className="text-gray-700 mr-6 bg-white/60 p-2 rounded-lg">{paiement.msg}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="md:col-span-2 bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                                        ...
                                        <div className="flex justify-center">
                                            <img
                                                src={'data:image/jpeg;base64,' + atob(paiement.preuve.split(',')[1])}
                                                alt="إثبات المعاملة"
                                                className="border border-amber-200 rounded-lg shadow-md max-w-xs cursor-pointer hover:opacity-90 transition-opacity"
                                                onClick={() => handleImageClick('data:image/jpeg;base64,' + atob(paiement.preuve.split(',')[1]))}
                                            />
                                        </div>
                                    </div>
                                </div>
