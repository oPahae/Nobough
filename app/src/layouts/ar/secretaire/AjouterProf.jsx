import {
    ArrowRight, Check, Phone, Mail, CreditCard, BookOpen, Trash2, Info, Plus, X, Sparkles, Moon, Upload
} from 'lucide-react'
import { useState, useRef } from 'react'
import Head from 'next/head'

export default function AjouterProf({ setActiveTab }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/profs/add', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                envoyerEmail(formData.email, data.password)
            } else {
                const data = await response.json()
                setErrors({ general: data.message || 'حدث خطأ أثناء إضافة الأستاذ' })
            }
        } catch (error) {
    ...
    const envoyerEmail = async (email, password) => {
        try {
            const res = await fetch(`/api/_mail/creerProf`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            <Head>
                <title>إضافة أستاذ - أكاديمية نبوغ</title>
            ...
            <div className="max-w-full relative" dir="rtl">
                ...
                <div className="mb-12 text-center pt-8 relative">
                    <div className="inline-block">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">إضافة أستاذ</h1>
                    ...
                    <p className="text-gray-600 mt-4 max-w-xl mx-auto">أضف أستاذًا جديدًا إلى الفريق التعليمي</p>
                </div>
                    ...
                    {submitted ? (
                        <div className="bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-xl mb-10 border border-amber-100">
                            <div className="flex flex-col items-center text-center">
                                ...
                                <h2 className="text-2xl font-bold text-gray-800 mb-4">تمت إضافة الأستاذ بنجاح!</h2>
                                <p className="text-gray-600 mb-8">تمت إضافة الأستاذ إلى قاعدة البيانات.</p>
                                <button
