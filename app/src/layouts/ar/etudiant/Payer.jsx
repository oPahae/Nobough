...
import { ribInfoAr } from '@/utils/Constants';
...
export default function Paiement({ session, setNotification }) {
        ...
        try {
            ...
            reader.onloadend = async () => {
                ...
                if (response.ok) {
                    setNotification((notif) => ({
                        ...notif,
                        msg: 'تم تقديم الدفع بنجاح! في انتظار التحقق.',
                        type: 'success',
                        shown: true
                    }))
                    fetchPaiements();
    ...
    return (
        <div dir="rtl">
            <Head>
                <title>الدفع عبر الإنترنت - أكاديمية نبوغ</title>
            ...
            <div className="max-w-full relative">
                <div className="mb-12 text-center pt-8 relative">
                    <div className="inline-block">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">الدفع عبر الإنترنت</h1>
                    ...
                    <p className="text-gray-600 mt-4 max-w-xl mx-auto">الرجاء إجراء الدفع الخاص بك عن طريق التحويل المصرفي باستخدام المعلومات أدناه</p>
                </div>
                    ...
                    <div className="max-w-full">
                        <div className="bg-white rounded-xl shadow-xl overflow-hidden p-8 relative mb-8">
                            <div className="mb-8 p-6 bg-amber-50 rounded-xl border border-amber-200 text-center">
                                <div className="flex items-center justify-center mb-2">
                                    <PaymentIcon className="h-8 w-8 text-amber-700 ml-2" />
                                    <h2 className="text-xl font-semibold text-gray-800">{moisAPayer ? `الشهر المختار: ${formatMois(moisAPayer.mois) + " " + moisAPayer.mois.split("/")[2]}` : 'المبلغ الإجمالي للدفع'}</h2>
                                </div>
                                <p className="text-4xl font-bold text-amber-700">{montantAPayer} درهم مغربي</p>
                            </div>
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="p-3 bg-amber-50/50 rounded-lg">
                                            <p className="text-sm font-medium text-amber-800">الحساب</p>
                                            <p className="text-gray-800 font-medium">{ribInfoAr.titulaire}</p>
                                        </div>
                                        <div className="p-3 bg-amber-50/50 rounded-lg">
                                            <p className="text-sm font-medium text-amber-800">البنك</p>
                                            <p className="text-gray-800 font-medium">{ribInfoAr.banque}</p>
                                        </div>
                                    </div>

                                    <div className="p-3 bg-amber-50/50 rounded-lg">
                                        <p className="text-sm font-medium text-amber-800 mb-2">الرقم المصرفي الدولي</p>
                                        <div className="flex items-center gap-2">
                                            <p className="text-gray-800 font-mono bg-white p-3 rounded-lg border border-amber-200 flex-grow">{ribInfoAr.rib}</p>
                                            <button
                                                onClick={() => copyToClipboard(ribInfoAr.rib)}
                                                className="p-3 rounded-lg hover:bg-amber-100 transition-colors"
                                                title="نسخ الرقم المصرفي الدولي"
                                            >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-2">
                                    <label htmlFor="preuve" className="block text-sm font-medium text-gray-700">إثبات الدفع</label>
                                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-amber-200 border-dashed rounded-xl bg-white/90 hover:bg-white transition-colors">
                                        <div className="space-y-1 text-center">
                                            ...
                                            <div className="flex text-sm text-gray-600">
                                                ...
                                                <p className="pr-1">أو اسحب وأفلِت</p>
                                            </div>
                                ...
                                <div className="flex justify-start">
                                    <button
                                        ...
                                        className="py-3 px-6 bg-gradient-to-l from-amber-600 to-amber-800 text-white rounded-xl shadow-md hover:shadow-lg transition-all transform hover:translate-x-0.5 flex items-center justify-center cursor-pointer disabled:opacity-50"
                                    >
                                        <span className="font-medium">تأكيد الدفع</span>
                                        {isSubmitting ? (
                    ...
                    <div className='max-w-full'>
                        <div className="space-y-2">
                            <select
                                ...
                                className="mt-1 block w-fit pr-3 pl-10 py-2 text-base bg-white mb-2 border-gray-300 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm rounded-md"
                            >
