...
export default function Login({ session, setNotification, setActiveTab }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/_auth/userLogin', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                localStorage.setItem('activeTabEtudiant', 'dashboard');
                window.location.reload();
            } else {
                setErrors({ general: data.message || 'خطأ أثناء تسجيل الدخول.' });
                setNotification((notif) => ({
                    ...notif,
    ...
    return (
        <div className="max-w-full relative pb-12 md:px-22" dir="rtl">
            ...
            <div className="mb-12 text-center pt-8 relative">
                ...
                <p className="text-gray-600 mt-4 max-w-xl mx-auto">قم بتسجيل الدخول للوصول إلى حسابك ومواصلة رحلتك الروحية</p>
            </div>
                ...
                {submitted ? (
                    ...
                    <form onSubmit={handleSubmit} className="bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-xl mb-10 border border-amber-100">
                        <div className="grid grid-cols-1 gap-6">
                            ...
                            {/* Password */}
                            <div className="mb-6">
                                <label className="block text-sm font-medium mb-2 text-gray-700">كلمة السر <span className="text-red-500">*</span></label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                        ...
                        <div className="text-center mt-4">
                            <p className="text-gray-600">
                                نسيت كلمة السر؟
                                <Link
                                    href="Etudiant/Recover" target='_blank'
                                    className="mr-2 text-amber-700 font-medium hover:text-amber-800 transition-colors cursor-pointer hover:underline"
                                >
                                    استعادة حسابك
                                </Link>
