    ...
    return (
        <div className="pt-4 pb-16" dir="rtl">
            <div className="flex flex-col md:flex-row justify-between items-center mb-4">
                <h2 className="text-xl md:text-2xl font-semibold text-amber-800">الدخل</h2>
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden border border-amber-100">
                <div className="p-4 border-b border-amber-100 flex justify-between items-center bg-gradient-to-r from-amber-50 to-white">
                    <div>
                        <h2 className="text-lg font-semibold text-amber-800">ملخص الدخل</h2>
                        {selectedDate && (
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-8 text-center text-sm text-gray-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p>لم يتم العثور على دخل لهذه الفترة</p>
                                            {selectedDate && (
                                                <button
                    ...
                    <div className="flex justify-between items-center">
                        <div className="text-left">
                            <span className="text-lg font-bold text-amber-800">
                                {filteredRevenus.reduce((sum, item) => sum + (typeof item.montant === 'string' ? parseFloat(item.montant) : item.montant), 0).toLocaleString()} درهم مغربي
                            </span>
                            <span className="text-xs text-amber-600 block">المجموع</span>
                        </div>
                        <span className="text-sm text-amber-700">عدد المعاملات: {filteredRevenus.length}</span>
                    </div>
                </div>
            </div>
        </div>
...
function FilterPopup({ filterPopupRef, selectedDate, setSelectedDate, setShowFilterPopup }) {
    return (
        <div
            ref={filterPopupRef}
            className="absolute left-0 top-full mt-2 bg-white rounded-md shadow-lg z-10 w-64 border border-amber-200"
        >
            <div className="p-3 border-b border-amber-100 flex justify-between items-center">
                ...
                <h3 className="text-sm font-medium text-amber-800">تصفية حسب التاريخ</h3>
            </div>
            <div className="p-3">
                <input
                    type="month"
        ...
        <tr className={`hover:bg-amber-50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-amber-50/30'}`}>
                ...
                <div className="flex items-start">
                    <div>
                        <div className="text-sm font-medium text-gray-900">
                            {revenu.type === 'inscriptions' && 'الاشتراكات'}
                            {revenu.type === 'paiements_mensuels' && 'المدفوعات الشهرية'}
                            {revenu.type === 'dons' && 'التبرعات'}
                        </div>
            ...
            <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900 flex items-center">
                    <Calendar className="w-4 h-4 ml-2 text-amber-500" />
                    {new Date(revenu.created_At).toLocaleDateString('FR-fr')}
                </div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-left">
                <span className="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                    +{revenu.montant.toLocaleString()} درهم مغربي
                </span>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-left">
                {revenu.descr}
