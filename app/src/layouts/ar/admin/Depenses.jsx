...
export default function ({ depenses, setDepenses, setActiveTab }) {
    ...
    const handleDeleteDepense = async (id) => {
        try {
            ...
            if (response.ok) {
                ...
                setErrors({ submit: data.message || 'خطأ أثناء حذف المصروف' });
            }
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br pt-6 pb-20" dir="rtl">
                        ...
                        <div className="mb-4 lg:mb-0">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-amber-600 text-lg">تتبع وتحليل مصروفاتك</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-amber-100/50">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="text-left">
                                    <div className="text-sm text-gray-500 font-medium">إجمالي المصروفات</div>
                            ...
                            {selectedDate && (
                                <div className="text-xs text-amber-600 flex items-center">
                                    ...
                                    الفترة: {new Date(selectedDate + '-01').toLocaleDateString('ar-EG', { month: 'long', year: 'numeric' })}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-amber-100/50">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="text-left">
                                    <div className="text-sm text-gray-500 font-medium">المعاملات</div>
                            ...
                            <div className="text-xs text-gray-500">
                                {filteredDepenses.length > 0 ? 'المصروفات المسجلة' : 'لا توجد معاملات'}
                            </div>
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-amber-100/50">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="text-left">
                                    <div className="text-sm text-gray-500 font-medium">الفئات</div>
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-amber-100/50">
                    <div className="p-6 bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-yellow-500/10 border-b border-amber-100">
                        <div className="flex justify-between items-center">
                            <div>
                                <h2 className="text-2xl font-bold text-amber-800 mb-1">جدول المصروفات</h2>
                                <p className="text-amber-600">إدارة ومراقبة جميع مصروفاتك</p>
                                {selectedDate && (
                                    <div className="mt-2 inline-flex items-center px-3 py-1 rounded-full text-xs bg-amber-100 text-amber-700">
                                        <Calendar className="w-3 h-3 ml-1" />
                                        {new Date(selectedDate + '-01').toLocaleDateString('ar-EG', { month: 'long', year: 'numeric' })}
                                    </div>
                                )}
                            </div>
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-6 py-16 text-center">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <h3 className="text-lg font-semibold text-gray-700 mb-2">لم يتم العثور على مصروفات</h3>
                                                <p className="text-gray-500 mb-4">
                                                    {selectedDate ? 'لا توجد مصروفات لهذه الفترة' : 'ابدأ بإضافة أول مصروف لك'}
                                                </p>
                                                {selectedDate ? (
                                                    <button
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center gap-2 text-amber-700">
                                        ...
                                        <span className="font-medium">المعاملات: {filteredDepenses.length}</span>
                                    </div>
                                        ...
                                        <div className="flex gap-2">
                                            {Object.entries(typeStats).map(([type, amount]) => (
                                                <div key={type} className="px-3 py-1 bg-white rounded-full text-xs font-medium text-amber-700 shadow-sm">
                                                    {type}: {amount.toLocaleString()} درهم
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                ...
                                <div className="text-left">
                                    <div className="text-sm text-amber-600 font-medium mb-1">إجمالي المصروفات</div>
...
function FilterPopup({ filterPopupRef, selectedDate, setSelectedDate, setShowFilterPopup }) {
    return (
        <div
            ref={filterPopupRef}
            className="absolute left-0 top-full mt-3 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl z-20 w-80 border border-amber-200/50 overflow-hidden"
        >
            <div className="p-4 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-b border-amber-100 flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-amber-800">تصفية حسب الفترة</h3>
                </div>
                <button
        ...
        <tr className={`group hover:bg-gradient-to-r hover:from-amber-50/50 hover:to-orange-50/50 transition-all duration-300 ${
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="text-sm font-medium text-gray-900">
                            {new Date(depense.created_At).toLocaleDateString('ar-EG', {
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric'
                            })}
                        ...
                        <div className="text-xs text-gray-500">
                            {new Date(depense.created_At).toLocaleDateString('ar-EG', { weekday: 'long' })}
                        </div>
                    </div>
                </div>
            </td>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-left">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl font-bold shadow-lg">
                    ...
                    -{depense.montant.toLocaleString()} درهم
                </div>
            </td>
