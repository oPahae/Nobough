...
export default function AjouterEvenement({ setActiveTab, budget, setNotification }) {
  ...
  const ajouterDepense = (e) => {
    ...
    if (!nouvelleDepense.label || !nouvelleDepense.montant) {
      setNotification((notif) => ({
        ...notif,
        msg: "Veuillez remplir au moins le libellé et le montant de la depense.",
        type: 'info',
        shown: true
      }))
      return;
  ...
  const handleSubmit = async (e) => {
    ...
    if(eventTotal >= budget.reel) {
      setNotification(notif => ({
        ...notif,
        msg: `Cet événement n'est pas réalisable, le budget réel insuffisant (${eventTotal} > ${budget.reel})`,
        type: 'error',
        shown: true
      }))
      return
              ...
              <div className="mb-4">
                {formData.depenses.length > 0 ? (
                      ...
                      >
                        ...
                        {dep.date && (
                          <div className="flex items-center text-amber-700 text-xs mt-2">
                            ...
                            <span>Date: {new Date(dep.date).toLocaleDateString('fr-FR', {
                              day: 'numeric',
                              month: 'long',
                              year: 'numeric'
                            })}</span>
                  ...
                  <div className="text-center py-4 bg-amber-50 rounded-lg border border-dashed border-amber-300">
                    <p className="text-amber-700">Aucune depense ajoutée</p>
                  </div>
                )}
              </div>

              {/* Formulaire d'ajout de depense */}
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h3 className="text-sm font-medium text-gray-700 mb-3">Ajouter une depense</h3>
