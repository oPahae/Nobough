import {
    ...
    Trash2, Edit, Plus, X, Upload, Info
} from 'lucide-react'
import { useState, useEffect, useRef } from 'react'

export default function ModifierAnnonce({ setActiveTab, setNotification }) {
    const fileInputRef = useRef(null)
    const [annonceID, setAnnonceID] = useState('')

    useEffect(() => {
        const temp = sessionStorage.getItem('annonce')
        if (!temp) setActiveTab('annonces')
        else setAnnonceID(temp)
    }, [])
    ...
    const validate = () => {
        ...
        if (!formData.descr.trim()) {
            newErrors.descr = "La description est requise"
        }
    ...
    const fetchAnnonce = async () => {
        ...
        try {
            setLoading(true)
            const response = await fetch(`/api/annonces/getOne?id=${annonceID}`)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('/api/annonces/update', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                const data = await response.json()
                setErrors({ general: data.message || 'Erreur lors de la modification de l\'annonce' })
            }
        } catch (error) {
        ...
        <div className="max-w-full relative">
            ...
            <div className="mb-12 text-center pt-8 relative">
                <div className="inline-block">
                    ...
                    <h1 className="text-3xl font-bold text-gray-800">Modifier une annonce</h1>
                ...
                <p className="text-gray-600 mt-4 max-w-xl mx-auto">Modifiez les informations de cette annonce</p>
            </div>
                ...
                {submitted ? (
                    <div className="bg-white/90 backdrop-blur-sm p-8 rounded-xl shadow-xl mb-10 border border-amber-100">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <h2 className="text-2xl font-bold text-gray-800 mb-4">Annonce modifiée avec succès !</h2>
                            <p className="text-gray-600 mb-8">Vos modifications ont été enregistrées.</p>
                            <button
                            ...
                            <div className="mb-6">
                                ...
                                <div className="relative">
                                    ...
                                    <textarea
                                        name="descr"
                                        rows="4"
                                        className={`block w-full pl-12 pr-4 py-4 border ${errors.descr ? 'border-red-300 bg-red-50' : 'border-amber-200'} rounded-xl bg-white/80 shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-600 focus:border-transparent text-gray-700`}
                                ...
                                {errors.descr && <p className="mt-2 text-sm text-red-600">{errors.descr}</p>}
                            </div>
                        ...
                        <div className="bg-amber-50 p-4 rounded-lg mb-8 flex">
                            ...
                            <div>
                                <p className="text-sm text-gray-700">
                                    En modifiant cette annonce, vous vous engagez à respecter les règles de communication de l'Académie.
                                </p>
                            </div>
                        </div>
