...
export default async function handler(req, res) {
  ...
  try {
    ...
    doc.setFontSize(12);
    doc.setTextColor(...colors.text);
    doc.text(`Professeur: ${professeur[0].prenom} ${professeur[0].nom}`, 148.5, 45, { align: 'center' });
    doc.setFontSize(10);
    doc.text(`Email: ${professeur[0].email || 'Non renseigné'} | Tél: ${professeur[0].tel || 'Non renseigné'} | CIN: ${professeur[0].cin || 'Non renseigné'}`, 148.5, 52, { align: 'center' });
    ...
    res.setHeader('Content-Disposition', `attachment; filename=emploi-du-temps-${professeur[0].prenom}-${professeur[0].nom}.pdf`);
    res.send(pdfBuffer);
