...
export default async function handler(req, res) {
    ...
    try {
        if (action === 'getCode') {
            const [rows] = await db.execute('SELECT * FROM Secretaires WHERE email = ?', [email]);
            if (rows.length === 0) {
        ...
        if (action === 'resetPassword') {
            ...
            await db.execute('UPDATE Secretaires SET password = ? WHERE email = ?', [hashedPassword, email]);
