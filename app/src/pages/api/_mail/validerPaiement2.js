...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        const [prof] = await db.execute('SELECT p.* FROM Professeurs p, Formations f WHERE p.id = f.profID AND f.id=?', [formationID]);

        try {
            const type = "validation"
            const msg = `Votre paiement de ${formation[0].titre} a été validée`
        ...
        try {
            const type = "adhesion"
            const msg = `Nouveau étudiant inscrit à <${formation[0].titre}>: "${`${etudiant[0].nom} ${etudiant[0].prenom}`}"`
            await db.execute('INSERT INTO NotificationsProf (profID, type, msg) VALUES (?, ?, ?)', [prof[0].id, type, msg]);
            res.status(201).json({ message: 'Notification envoyée' });
        } catch (error) {
            console.error('Erreur lors de l\'envoi de la notification:', error);
            res.status(500).json({ message: 'Erreur serveur' });
            ...
            await transporter.sendMail({
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; color: #333; background-color: #f9f9f9; padding: 20px; border-radius: 8px; max-width: 600px; margin: auto; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                        ...
                        <p style="font-size: 16px; line-height: 1.5; text-align: center;">
                            On vous informe que votre paiement pour la formation <<${formation[0].titre}>> a été validé avec succès, vous pouvez maintenant vous connecter dans votre formation.
                        </p>
