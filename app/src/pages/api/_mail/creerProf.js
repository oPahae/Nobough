...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        if (!email || !password) return res.status(400).json({ message: 'Infos Manquantes' });
        console.log(email)
        ...
        try {
            ...
            await transporter.sendMail({
                ...
                to: email,
                subject: 'Compte Créé',
                text: `Votre compte professeur a été créé`,
                html: `
                    <div style="font-family: Arial, sans-serif; color: #333; background-color: #f9f9f9; padding: 20px; border-radius: 8px; max-width: 600px; margin: auto; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                        ...
                        <p style="font-size: 16px; line-height: 1.5; text-align: center;">
                            On vous informe que votre compte professeur a été créé avec succès, vous pouvez maintenant vous connecter avec ce mot de passe :
                            <p style="font-size: 16px; font-weight: bold; color: #4CAF50;">
                                ${password}
                            </p>
                            Ne le communiquez avec aucune personne, vous avez la possibilité de le modifier dans votre espace.
                        </p>
