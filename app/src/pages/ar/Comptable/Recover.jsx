...
export default function Forgot({ session }) {
    ...
    useEffect(() => {
        if(session) router.push('/ar/Comptable')
    })
    ...
    const fetchCode = async () => {
        try {
            const response = await fetch('/api/_auth/comptableRecover', {
                method: 'POST',
                headers: {
    ...
    return (
        <>
            <Head>
                <title>نسيت كلمة المرور - أكاديمية نبوغ</title>
                        ...
                        <div className="relative w-full max-w-md bg-white p-8 rounded-xl shadow-lg overflow-hidden">
                            ...
                            <div ref={formRef} className="relative">
                                ...
                                <div className="text-center mt-4">
                                    <Link href="/ar/Comptable" className="text-amber-600 hover:underline">
                                        العودة
                                    </Link>
                                </div>
                            </div>
