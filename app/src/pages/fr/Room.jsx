...
export default function Room({ session }) {
    ...
    const roomCode = router.query.code || 'LOADING...';
    ...
    useEffect(() => {
        if (!session || roomCode === 'LOADING...') return;
    ...
    const initializeWebRTC = async () => {
        try {
            socketRef.current = io(process.env.EXPRESS_PORT || 'http://localhost:5000');
            await getUserMedia();
            setupSocketEvents();
    ...
    const handleIceCandidate = async (candidate, senderSocketId) => {
        try {
            ...
            console.error('Erreur lors de l\'ajout du ICE candidate:', error);
        }
    };
...
const handleStopScreenShare = async () => {
    ...
    return (
        <>
            ...
            {/* Code */}
            <div className="flex justify-end mb-6 fixed bottom-4 right-4 z-50">
                <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl px-6 py-3 shadow-2xl">
                    <div className="flex items-center gap-3">
                        <span className="text-white/80 text-sm font-medium">Code de la salle:</span>
                        ...
                        <div className="relative w-full h-96 bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl overflow-hidden shadow-inner flex items-center justify-center">
                            {session.role === 'professeur' ? (
                                ...
                                <div className="text-white/50 text-center">
                                    <div className="text-lg mb-2">En attente du professeur...</div>
                                    <div className="text-sm">
                                        {professor ? `${professor.prenom} ${professor.nom}` : 'Aucun professeur connecté'}
                                    </div>
                                </div>
                            )}

                            {/* Indicateur de micro pour le professeur */}
                            {professor && (
                                ...
                                {participants.map((participant) => (
                                    ...
                                    >
                                        <div className="aspect-square relative mb-2">
                                            ...
                                            {/* Statut audio */}
                                            <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-white shadow-sm flex items-center justify-center ${participant.isMuted ? 'bg-red-500' : 'bg-green-500'
                                                }`}>
                                                {participant.isMuted ? (
                                            ...
                                            {participant.isHandRaised && (
                                                <div className="absolute -top-1 -left-1 w-6 h-6 bg-yellow-500 rounded-full border-2 border-white shadow-sm flex items-center justify-center">
                                        ...
                                        {/* Audio element pour les streams distants */}
