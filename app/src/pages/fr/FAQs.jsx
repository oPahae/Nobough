...
export default function FAQ() {
        ...
        try {
            ...
            if (data.length > 0) {
                formattedFaqs = data.map(faq => ({
                    ...
                    reponse: faq.reponse,
                    created_at: new Date(faq.created_at).toLocaleDateString('fr-FR')
                }))
            }
    ...
    return (
        <>
            <Head>
                <title>FAQ - Académie Nobough</title>
                <link rel="icon" href="/logo2-nobg.png" />
            </Head>

            <div className="w-full h-screen overflow-y-scroll relative bg-white/60 backdrop-blur-3xl">
                ...
                <div className="mb-12 text-center pt-8 relative">
                    <div className="inline-block">
                        ...
                        <h1 className="text-3xl font-bold text-gray-800">FAQ - Questions Fréquentes</h1>
                    ...
                    <p className="text-gray-600 mt-4 max-w-xl mx-auto">Trouvez des réponses à vos questions les plus fréquentes</p>
                </div>
                            ...
                            {filteredFaqs.length > 0 ? (
                                filteredFaqs.map((faq) => (
                                    <div key={faq.id} className="border border-amber-100 rounded-xl overflow-hidden">
                                        <button
                                            className="w-full p-6 text-left bg-white hover:bg-amber-50 transition-colors flex justify-between items-center"
                                            onClick={() => toggleExpand(faq.id)}
                                        >
                                            <div className="flex items-center">
                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-amber-700 flex items-center justify-center shadow-md mr-4">
                                        ...
                                        {expandedItems[faq.id] && (
                                            <div className="p-6 bg-amber-50/50 border-t border-amber-100">
                                                ...
                                                <p className="text-xs text-gray-500 mt-4">Ajoutée le {faq.created_at}</p>
                                            </div>
                                        )}
                                    </div>
                                ))
